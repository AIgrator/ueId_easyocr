...
def run_uied_pipeline(screenshot_path, temp_dir, ocr_reader):
    ...
    try:
        ...
        logging.info(f"Component Filtering: Original: {len(raw_components)}, Non-Text: {len(non_text_components)}")

        # 4. Save the filtered components
        file.save_corners_json(raw_compo_json_path, non_text_components)
        draw.draw_bounding_box(org_img, non_text_components, write_path=ip_img_path, show=False)
        cpu_readings.append(psutil.cpu_percent(interval=None))
