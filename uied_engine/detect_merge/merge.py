...
from uied_engine.detect_merge.Element import Element
...
def show_elements(org_img, eles, show=False, win_name='element', wait_key=0, shown_resize=None, line=2):
    ...
    img = org_img.copy()
    for ele in eles:
        # Use a default color if the category is not in the map, to avoid KeyErrors
        color = color_map.get(ele.category, (0, 0, 0)) # Default to black
...
def merge(img_path, compo_path, text_path, output_json_path, is_paragraph=False, is_remove_bar=True, show=False, wait_key=0):
    # The directory of the output file is extracted to save the visualization image
    merge_root = os.path.dirname(output_json_path)
