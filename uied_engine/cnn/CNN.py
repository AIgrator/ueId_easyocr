...
from uied_engine.config.CONFIG import Config
cfg = Config()
...
class CNN:
    ...
    def load(self, classifier_type):
        if classifier_type == 'Text':
            self.model_path = cfg.MODEL_PATH_TEXT
            self.class_map = cfg.CLASS_MAP_TEXT
        elif classifier_type == 'Noise':
            self.model_path = cfg.MODEL_PATH_NOISE
            self.class_map = cfg.CLASS_MAP_NOISE
        elif classifier_type == 'Elements':
            self.model_path = cfg.CNN_PATH
            self.class_map = cfg.element_class
            self.image_shape = (64, 64, 3)
        elif classifier_type == 'Image':
            self.model_path = cfg.MODEL_PATH_IMAGE
            self.class_map = cfg.CLASS_MAP_IMAGE
        
        self.class_number = len(self.class_map)
        
        # Build the model architecture first, without compiling it for training
        self.build_model(epoch_num=0, is_compile=False)
        # Then, load only the learned weights into the newly created architecture
    ...
    def predict(self, imgs, compos, load=False, show=False):
        ...
        for i in range(len(imgs)):
            ...
            # The model expects a batch of images, so we predict on a list containing one image
            prediction = self.model.predict(X)
            Y = self.class_map[np.argmax(prediction)]
            compos[i].category = Y
            if show:
                print(Y)
