from uied_engine.detect_text import ocr
from uied_engine.detect_text.Text import Text
import numpy as np
import cv2
import json
import time
...
def text_detection(input_file, output_json_path, ocr_reader=None, show=False):
    '''
    :param input_file: path to the image
    :param output_json_path: path to the output json file
    :param ocr_reader: the preloaded easyocr model
    '''
    start = time.perf_counter()
    # The directory of the output file is extracted to save the visualization image
    output_root = os.path.dirname(output_json_path)
    name = os.path.basename(output_json_path).replace('.json', '')
    
    img = cv2.imread(input_file)

    print('*** Detect Text through EasyOCR ***')
    # The OCR function now directly saves the output to the specified path
    ocr.ocr_detection_easyocr(input_file, ocr_reader, output_json_path)
    
    if not os.path.exists(output_json_path):
        print(f"ERROR: OCR result file not found at {output_json_path}")
        return
    ...
    visualize_texts(img, texts, shown_resize_height=800, show=show, write_path=pjoin(output_root, name + '.png'))
    # Overwrite the ocr result with the processed texts
    save_detection_json(output_json_path, texts, img.shape)
    print("[Text Detection Completed in %.3f s] Input: %s Output: %s" % (time.perf_counter() - start, input_file, output_json_path))
